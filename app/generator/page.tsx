...
export default function LinkGenerator() {
  ...
  // PERBAIKAN: Menggunakan encodeURIComponent agar simbol & atau karakter unik lainnya aman
  ...
  const shareToWhatsApp = () => {
    if (!name) return;
    // Teks pesan WA dengan format bold pada nama tamu
    ...
    <div className="min-h-screen bg-slate-900 p-8 flex items-center justify-center font-sans">
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-800">Undangan Generator</h1>
            <p className="text-xs text-slate-500">Radit & Keiani Wedding</p>
          </div>
        </div>

        <div className="space-y-5">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nama Tamu</label>
            <input
              type="text"
          ...
          {name && (
            <div className="p-4 bg-slate-50 border border-dashed border-slate-300 rounded-xl">
              <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Generated Link Preview:</p>
              <p className="text-xs font-mono text-blue-600 break-all leading-relaxed">{finalLink}</p>
            </div>
          )}
